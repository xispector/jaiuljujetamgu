const rotatingDonut = () => {
  ...
  const yRotationMatrix = (x, y, z, theta) => [
    x * cos(theta) - z * sin(theta),
    y,
    x * sin(theta) + z * cos(theta),
  ];
  const xRotationMatrix = (x, y, z, theta) => [
    x,
    y * cos(theta) - z * sin(theta),
    y * sin(theta) + z * cos(theta),
  ];
  const zRotationMatrix = (x, y, z, theta) => [
  ...
  const point = (x, y, z) => {
    ...
    return (
      origin +
      Math.round((K1 * x) / (K2 + z)) -
      80 * Math.round((K1 * y) / (K2 + z))
    );
  };
  ...
  setInterval(() => {
    ...
    const zBuffer = [...Array(resolution)].map((_, i) =>
      i % 80 == 79 ? "\n" : "0"
    );
    ...
    for (let phi = 0; phi < 6.28; phi += 0.1) {
      for (let theta = 0; theta < 6.28; theta += 0.1) {
        ...
        position = position.map(
          (_, idx, arr) => yRotationMatrix(arr[0], arr[1], arr[2], phi)[idx]
        );
        position = position.map(
          (_, idx, arr) => xRotationMatrix(arr[0], arr[1], arr[2], A)[idx]
        );
        position = position.map(
          (_, idx, arr) => zRotationMatrix(arr[0], arr[1], arr[2], B)[idx]
        );
        ...
        const lightIndex = light.reduce(
          (sum, v, i) => sum + v * gradient[i] * 8
        );
        if (
          zBuffer[point(position[0], (9 / 19) * position[1], position[2])] <
          1 / (position[2] + K2)
        ) {
          if (lightIndex > 0) {
            a[point(position[0], (9 / 19) * position[1], position[2])] =
              lightAscii[Math.floor(lightIndex)];
          } else {
            a[point(position[0], (9 / 19) * position[1], position[2])] = ".";
          }
          zBuffer[point(position[0], (9 / 19) * position[1], position[2])] =
