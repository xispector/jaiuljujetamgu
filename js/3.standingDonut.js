...
const point = (x, y, z) => {
  ...
  for (let phi = 0; phi < 6.28; phi += .1) {
    for (let theta = 0; theta < 6.28; theta += .1) {
      ...
      position = position.map((_, idx, arr) => xRotationMatrix(arr[0], arr[1], arr[2], Math.PI / 2)[idx]);
      gradient = gradient.map((_, idx, arr) => yRotationMatrix(arr[0], arr[1], arr[2], phi)[idx]);
      gradient = gradient.map((_, idx, arr) => xRotationMatrix(arr[0], arr[1], arr[2], Math.PI / 2)[idx]);
